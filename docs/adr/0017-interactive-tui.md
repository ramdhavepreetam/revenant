...
- **Status:** In progress · Phase A (V0–V2) Implemented · Phase B (V3–V5) pending
...
## Acceptance criteria
- [ ] `revenant chat --tui` opens a full-screen app: persistent input box, live
      streaming activity log, status bar (model · workspace · mode · context gauge).
- [ ] Typing `/` shows a discoverable menu of commands **and** skills with
      descriptions; selecting one runs it.
- [ ] Sub-agents spawned during a run are **visible** as nested/labelled lanes.
- [ ] A live **context-size** gauge updates as the transcript grows and flags folds.
- [ ] `ctrl-c` interrupts a running goal without quitting; `ctrl-d` quits; ↑/↓
      history works.
- [ ] Without textual (or piped/CI/`NO_COLOR`): falls back to today's REPL,
      byte-identical. `AgentEvent` changes are additive — Plain/Rich unaffected.
- [ ] Frozen binary loads textual or falls back cleanly.
- [ ] Suite green (bare `pytest`); ADR-0017 + README + CHANGELOG updated per slice.
